...
static int __init blk_ram_init(void)
{
	...
	blk_ram_dev->tag_set.flags = 0; /* Remove BLK_MQ_F_SHOULD_MERGE for newer kernels */
	...
	/* Allocate disk with 3 parameters for newer kernel */
	disk = blk_ram_dev->disk = blk_mq_alloc_disk(&blk_ram_dev->tag_set, &lim, blk_ram_dev);
	...
	pr_info("module loaded: capacity=%luMB, logical_block_size=%lu, physical_block_size=%lu\n",
		capacity_mb, lbs, pbs);
	return 0;

cleanup_minor:
	ida_free(&blk_ram_indexes, minor);
cleanup_disk:
	put_disk(blk_ram_dev->disk);
tagset_err:
	blk_mq_free_tag_set(&blk_ram_dev->tag_set);
	kvfree(blk_ram_dev->data);
data_err:
	kfree(blk_ram_dev);
unregister_blkdev:
...
{
	int minor = blk_ram_dev->disk->first_minor;
